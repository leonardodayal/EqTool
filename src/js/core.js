(function (root, factory) {
  ...
  function astTex(node) {
    if (!node) { return ''; }
    if (node.t === 'num') { return node.v; }
    if (node.t === 'var') { return varTex(node.v); }
    if (node.t === 'paren') { return '\\left(' + astTex(node.inner) + '\\right)'; }
    if (node.t === 'unary') { return '-' + astTex(node.arg); }
    if (node.t === 'other') { return node.v; }

    if (node.t === 'func') {
      const name = node.name;
      const a0 = node.args[0] ? astTex(node.args[0]) : '';
      const all = node.args.map(astTex).join(', ');
    ...
    if (node.t === 'binop') {
      ...
      if ((op === '^' || op === '.^') && left.t === 'func' && TRIG_POW.has(left.name)) {
        ...
        return fn + '^{' + astTex(right) + '}\\left(' + left.args.map(astTex).join(', ') + '\\right)';
      }

      if (op === '/' || op === './') {
        return '\\dfrac{' + astTex(left) + '}{' + astTex(right) + '}';
      }

      if (op === '^' || op === '.^') {
        let base = astTex(left);
        if (left.t === 'binop' || left.t === 'func') {
          base = '\\left(' + astTex(left) + '\\right)';
        }
        let exp;
        if (
          ...
          exp = '\\frac{' + astTex(right.inner.left) + '}{' + astTex(right.inner.right) + '}';
        } else {
          exp = astTex(right);
        }
        return base + '^{' + exp + '}';
      ...
      if (op === 'back') { return astTex(left) + ' \\backslash ' + astTex(right); }
      if (op === '+' || op === '-') { return astTex(left) + ' ' + op + ' ' + astTex(right); }
      if (op === '=') { return astTex(left) + ' = ' + astTex(right); }
    }
